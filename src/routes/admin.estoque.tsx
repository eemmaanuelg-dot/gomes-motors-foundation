...
function AdminStockPage() {
  ...
  const action = async (vehicle: Vehicle, actionName: string, payload: Record<string, unknown>) => {
    ...
    try {
      const response = await fetch("/admin/api", { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, credentials: "include", body: JSON.stringify({ action: actionName, ...payload }) });
...
function VehicleRow({ vehicle, saving, onAction }: { vehicle: Vehicle; saving: boolean; onAction: (vehicle: Vehicle, action: string, payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <div className="rounded-sm border border-border bg-card p-4 sm:p-5"><div className="grid gap-4 lg:grid-cols-[1.5fr_160px_170px_180px_auto] lg:items-end">
    ...
    <div className="flex gap-2"><Link to="/admin/editar-veiculo/$id" params={{ id: vehicle.id }} className="inline-flex h-9 items-center justify-center gap-2 rounded-sm border border-border px-3 text-sm font-semibold hover:bg-secondary"><Pencil className="h-4 w-4" />Editar</Link><button disabled={saving} onClick={() => { const cents = Math.round(Number(price.replace(".", "").replace(",", ".")) * 100); void onAction(vehicle, "updateVehicle", { id: vehicle.id, data: { brand: vehicle.brand, model: vehicle.model, version: vehicle.version ?? null, year: vehicle.year ?? null, mileage: vehicle.mileage ?? 0 } }).then(() => onAction(vehicle, "setPrice", { id: vehicle.id, priceCents: cents })).then(() => onAction(vehicle, "setStatus", { id: vehicle.id, status: status as "disponivel" | "reservado" | "vendido" })).then(() => onAction(vehicle, "setInventory", { id: vehicle.id, published: published && status !== "vendido", order: vehicle.displayOrder ?? 0 })); }} className="inline-flex h-9 items-center justify-center gap-2 rounded-sm bg-foreground px-4 text-sm font-semibold text-background disabled:opacity-50"><Save className="h-4 w-4" />{saving ? "Salvando…" : "Salvar"}</button></div>
