...
type ReportData = {
  ...
  leads: { intent: string; status: string; total: number }[];
  ...
  analytics: { event_name: string; total: number }[];
};
...
function ReportsPage() {
  ...
  if (error) return <Shell><Panel><p className="text-sm text-brand-red">{error}</p><button onClick={() => void load()} className="mt-4 rounded-sm border border-border px-4 py-2 text-sm font-semibold">Tentar novamente</button></Panel></Shell>;
  if (!data) return null;
  return <Shell><div className="space-y-6"><header><p className="text-xs font-bold uppercase tracking-[0.18em] text-gold">Painel administrativo</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Relatórios operacionais</h1><p className="mt-2 text-sm text-muted-foreground">Indicadores consolidados do estoque, funil comercial, financiamento e comportamento do site.</p></header><div className="grid gap-4 sm:grid-cols-2"><Stat title="Vendas registradas" value={String(data.sales.total)} /><Stat title="Faturamento bruto" value={money(data.sales.gross_cents)} /></div><div className="grid gap-6 lg:grid-cols-2">{section("Estoque por status", data.stock.map((r) => ({ label: labels[r.status] ?? r.status, value: r.total })))}{section("Leads por status", data.leads.reduce((acc, r) => { const key = labels[r.status] ?? r.status; const current = acc.find((item) => item.label === key); if (current) current.value += r.total; else acc.push({ label: key, value: r.total }); return acc; }, [] as { label: string; value: number }[]))}{section("Leads por intenção", data.leads.reduce((acc, r) => { const key = labels[r.intent] ?? r.intent; const current = acc.find((item) => item.label === key); if (current) current.value += r.total; else acc.push({ label: key, value: r.total }); return acc; }, [] as { label: string; value: number }[]))}{section("Avaliações de veículos", data.evaluations.map((r) => ({ label: labels[r.decision] ?? r.decision, value: r.total })))}{section("Negociações", data.negotiations.map((r) => ({ label: labels[r.stage] ?? r.stage, value: r.total })))}{section("Reservas", data.reservations.map((r) => ({ label: labels[r.status] ?? r.status, value: r.total })))}{section("Financiamento", data.financing.map((r) => ({ label: labels[r.status] ?? r.status, value: r.total })))}{section("Analytics — últimos 30 dias", data.analytics.map((r) => ({ label: labels[r.event_name] ?? r.event_name, value: r.total })))}</div></div></Shell>;
}
