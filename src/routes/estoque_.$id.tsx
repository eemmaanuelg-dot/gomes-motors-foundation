    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        ...
        <section className="rounded-sm border border-border bg-card p-6 sm:p-8">
          ...
          <div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-sm border border-border bg-border">{[["Ano", String(veiculo.ano)], ["Quilometragem", formatarKm(veiculo.km)], ...(veiculo.cambio ? [["Câmbio", veiculo.cambio]] : []), ...(veiculo.combustivel ? [["Combustível", veiculo.combustivel]] : []), ...(veiculo.categoria === "carros" && veiculo.fichaTecnica.motor ? [["Motor", veiculo.fichaTecnica.motor]] : []), ...(veiculo.cilindrada ? [["Cilindrada", veiculo.cilindrada]] : []), ...(veiculo.tipo ? [["Tipo", veiculo.tipo]] : [])].map(([label, value]) => <div key={label} className="bg-card p-4"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{label}</p><p className="mt-1 text-sm font-semibold text-foreground">{value}</p></div>)}</div>
