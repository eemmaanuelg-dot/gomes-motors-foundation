...
async function audit(db: D1DatabaseLike, request: Request, action: string, entityId: string | null, result: "success" | "failure", metadata: Record<string, unknown> = {}) {
  await db.prepare(`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, result, occurred_at, metadata_json) VALUES (?, ?, ?, 'vehicle', ?, ?, ?, ?)`).bind(crypto.randomUUID(), actor(request), action, entityId, result, new Date().toISOString(), JSON.stringify(metadata)).run();
}
...
function integer(value: unknown, field: string, minimum = 0) {
  ...
  if (!Number.isInteger(parsed) || parsed < minimum) throw new Error(`${field} inválido.`);
  return parsed;
}
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const brand = requiredText(body.brand, "Marca");
          const model = requiredText(body.model, "Modelo");
          const version = String(body.version ?? "").trim() || null;
          const year = integer(body.year, "Ano", 1900);
          ...
          const mileage = integer(body.mileage, "Quilometragem");
          const priceCents = integer(body.priceCents, "Preço", 0);
          const status = requiredText(body.status || "disponivel", "Status");
          const description = String(body.description ?? "").trim() || null;
          const transmission = String(body.transmission ?? "").trim() || null;
          const fuel = String(body.fuel ?? "").trim() || null;
          const color = String(body.color ?? "").trim() || null;
          const cylinderCapacity = String(body.cylinder_capacity ?? "").trim() || null;
          const vehicleType = String(body.vehicle_type ?? "").trim() || null;
          const published = Boolean(body.published) && status !== "vendido" ? 1 : 0;
          const featured = Boolean(body.featured) ? 1 : 0;
          const displayOrder = integer(body.displayOrder ?? 0, "Ordem", 0);
          const financing = typeof body.financing === "object" && body.financing !== null ? body.financing as Record<string, unknown> : {};
          const entradaMinima = integer(financing.entradaMinima ?? 0, "Entrada mínima", 0);
          const taxaIndicativa = Number(financing.taxaIndicativa ?? 1.89);
          ...
          if (!ALLOWED_CATEGORY.has(category)) throw new Error("Categoria inválida. Use carros ou motos.");
          if (!ALLOWED_STATUS.has(status)) throw new Error("Status inválido.");
          if (!Number.isFinite(taxaIndicativa) || taxaIndicativa < 0) throw new Error("Taxa indicativa inválida.");
          if (!parcelas.length) throw new Error("Informe pelo menos um prazo de financiamento.");

          vehicleId = requiredText(body.id || `${brand}-${model}-${year}-${crypto.randomUUID().slice(0, 8)}`.toLowerCase().replace(/[^a-z0-9]+/g, "-"), "ID");
          const now = new Date().toISOString();
          const existing = await db.prepare(`SELECT id FROM vehicles WHERE id = ? LIMIT 1`).bind(vehicleId).first<{ id: string }>();
          if (existing) throw new Error("Já existe um veículo com este ID.");
          ...
          await audit(db, request, "vehicle.create", vehicleId, "success", { category, brand, model, year, priceCents, status, published: Boolean(published) });
          return json({ ok: true, id: vehicleId });
        } catch (error) {
          const message = error instanceof Error ? error.message : "Não foi possível cadastrar o veículo.";
          try { await audit(db, request, "vehicle.create", vehicleId, "failure", { message }); } catch { /* Auditoria não deve mascarar o erro original. */ }
