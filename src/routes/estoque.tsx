...
function correspondeFaixaPreco(preco: number, filtro: string) {
  switch (filtro) {
    ...
    case "R$ 80 mil a R$ 110 mil":
      return preco > 80000 && preco <= 110000;

    case "Acima de R$ 110 mil":
      return preco > 110000;

    case "Até R$ 32 mil":
      return preco <= 32000;

    case "Acima de R$ 32 mil":
      return preco > 32000;
...
function correspondeFaixaKm(km: number, filtro: string) {
  switch (filtro) {
    ...
    case "40.000 a 60.000 km":
      return km > 40000 && km <= 60000;

    case "Acima de 60.000 km":
      return km > 60000;

    case "Até 20.000 km":
      return km <= 20000;

    case "Acima de 20.000 km":
      return km > 20000;
...
function correspondeCilindrada(
  ...
  if (filtro === "Até 400 cc") {
    return valor <= 400;
  }

  if (filtro === "Acima de 400 cc") {
    return valor > 400;
  }
...
function FiltrosLaterais({
  ...
  const visiveis = expandido
    ? filtrosDisponiveis
    : filtrosDisponiveis.slice(0, VISIVEIS_INICIAL);
    ...
    <div className="space-y-4">
      {visiveis.map((filtro) => {
        ...
        return (
          <FiltroSelect
            ...
            valor={filtros[chave]}
            onChange={(valor) => onChange(chave, valor)}
          />
        );
      })}
      ...
      {filtrosDisponiveis.length > VISIVEIS_INICIAL && (
        ...
        >
          {expandido
            ? "Ver menos filtros"
            : `Mais filtros (${filtrosDisponiveis.length - VISIVEIS_INICIAL})`}
...
function CardVeiculo({
  ...
  if (veiculo.cambio) {
    detalhes.push(veiculo.cambio);
  }

  if (veiculo.combustivel) {
    detalhes.push(veiculo.combustivel);
  }

  if (veiculo.cilindrada) {
    detalhes.push(veiculo.cilindrada);
  }

  if (veiculo.tipo) {
    detalhes.push(veiculo.tipo);
  }
    ...
    <article className="group overflow-hidden rounded-sm border border-border bg-card">
      ...
      <div className="p-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <button
            type="button"
            className="rounded-sm bg-brand-red px-4 py-2.5 text-sm font-semibold text-brand-red-foreground transition-opacity hover:opacity-90"
          >
            Tenho interesse
          </button>
        </div>
...
function EstoquePage() {
  ...
  const veiculos = useMemo(() => {
    ...
    const filtrados = VEICULOS.filter((veiculo) => {
      ...
      if (
        filtros.ano &&
        veiculo.ano !== Number(filtros.ano)
      ) {
        return false;
      }
      ...
      if (
        filtros.combustivel &&
        veiculo.combustivel !== filtros.combustivel
      ) {
        return false;
      }
      ...
      <div className="mt-8 grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
        ...
        <label className="relative block">
          ...
          <select
            ...
            onChange={(event) =>
              setOrdenacao(
                event.target.value as Ordenacao,
              )
            }
          ...
          >
            ...
            <option value="menor-km">
              Menor quilometragem
            </option>
            <option value="maior-km">
              Maior quilometragem
            </option>
        ...
        <section aria-label="Veículos do estoque">
          ...
          {veiculos.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {veiculos.map((veiculo) => (
                <CardVeiculo
                  key={veiculo.id}
                  veiculo={veiculo}
                  favorito={favoritos.has(veiculo.id)}
                  onAlternarFavorito={() =>
                    alternarFavorito(veiculo.id)
                  }
                />
              ))}
            </div>
            ...
            <div className="rounded-sm border border-border bg-card px-6 py-14 text-center">
              <h2 className="text-lg font-semibold text-foreground">
                Não encontramos veículos com esses
                critérios.
              </h2>

              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Tente remover alguns filtros ou realizar
                uma nova busca.
              </p>
