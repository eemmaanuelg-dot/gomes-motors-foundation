...
function obterFiltros(categoria: Categoria, filtros: Filtros): FiltroGrupo[] {
  ...
  const grupos: FiltroGrupo[] = [
    ...
    {
      grupo: "Ano",
      opcoes: [...new Set(base.map((veiculo) => String(veiculo.ano)))].sort(
        (a, b) => Number(b) - Number(a),
      ),
    },
    {
      grupo: "Preço",
      opcoes: categoria === "motos" ? FAIXAS_PRECO_MOTOS : FAIXAS_PRECO_CARROS,
    },
    {
      grupo: "Quilometragem",
      ...
      <div className="p-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center gap-1.5 rounded-sm bg-brand-red px-4 py-2.5 text-sm font-semibold text-brand-red-foreground transition-opacity hover:opacity-90"
          >
  ...
  const veiculos = useMemo(() => {
    ...
    const filtrados = VEICULOS.filter((veiculo) => {
      ...
      if (termoBusca) {
        ...
        if (!camposBusca.some((valor) => valor.toLocaleLowerCase("pt-BR").includes(termoBusca))) {
          return false;
        }
      }
    ...
    switch (ordenacao) {
      ...
      case "menor-km":
        return [...filtrados].sort((a, b) => a.km - b.km);
      case "maior-km":
        return [...filtrados].sort((a, b) => b.km - a.km);
      case "mais-novo":
        return [...filtrados].sort((a, b) => b.ano - a.ano);
      case "mais-antigo":
        return [...filtrados].sort((a, b) => a.ano - b.ano);
      default:
        return filtrados;
    }
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {veiculos.map((veiculo) => (
                <CardVeiculo
                  ...
                  onAlternarFavorito={() => alternarFavorito(veiculo.id)}
                />
              ))}
            </div>
          ) : (
