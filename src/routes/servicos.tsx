...
export const Route = createFileRoute("/servicos")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Comprar, vender, trocar, consignar e financiar: escolha um serviço e envie seus dados para atendimento da Gomes Motors." },
      { property: "og:title", content: "Serviços — Gomes Motors" },
      { property: "og:description", content: "Fluxos comerciais para comprar, vender, trocar, consignar e financiar veículos." },
...
function SelectCampo({ label, name, options, required = false }: { label: string; name: string; options: string[]; required?: boolean }) {
  return <label className="block"><span className={labelClass}>{label}{required && " *"}</span><select className={inputClass} name={name} required={required} defaultValue=""><option value="">Selecione</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}

function TextareaCampo({ label, name, placeholder }: { label: string; name: string; placeholder?: string }) {
  return <label className="block sm:col-span-2"><span className={labelClass}>{label}</span><textarea className={`${inputClass} min-h-24 resize-y`} name={name} placeholder={placeholder} /></label>;
}
  ...
  return (
    <label className="block sm:col-span-2">
      ...
      <span className="flex cursor-pointer items-center gap-2 rounded-sm border border-dashed border-border bg-secondary px-3 py-3 text-sm text-muted-foreground hover:border-gold hover:text-foreground">
        <Upload className="h-4 w-4 text-gold" /> Selecionar fotos
...
function VeiculoSelector({ value, onChange, label = "Veículo desejado" }: { value: string; onChange: (value: string) => void; label?: string }) {
  ...
  const filtrados = disponiveis.filter((veiculo) => `${veiculo.marca} ${veiculo.modelo} ${veiculo.versao ?? ""}`.toLowerCase().includes(busca.toLowerCase()));
  return (
    <div className="space-y-3">
      <label className="block"><span className={labelClass}>{label} *</span><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input className={`${inputClass} pl-9`} value={busca} onChange={(event) => setBusca(event.target.value)} placeholder="Buscar por marca ou modelo" /></div></label>
...
function BotaoWhatsApp({ texto, disabled = false }: { texto: string; disabled?: boolean }) {
  return <button type="submit" disabled={disabled} className="inline-flex w-full items-center justify-center gap-2 rounded-sm bg-brand-red px-5 py-3 text-sm font-semibold text-brand-red-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"><MessageCircle className="h-4 w-4" />{texto}</button>;
}

function Roteiro({ numero, texto }: { numero: string; texto: string }) {
  return <div className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gold/10 text-xs font-bold text-gold">{numero}</span><p className="pt-1 text-sm leading-6 text-muted-foreground">{texto}</p></div>;
}

function ServicoBloco({ id, icon: Icon, title, detalhe, children }: { id: string; icon: typeof Car; title: string; detalhe: string; children: ReactNode }) {
  return <article id={id} className="scroll-mt-24 border border-border bg-card"><div className="border-b border-border p-6 sm:p-8"><div className="flex items-center gap-3"><Icon className="h-7 w-7 text-gold" /><h2 className="text-2xl font-bold text-foreground">{title}</h2></div><p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">{detalhe}</p></div><div className="p-6 sm:p-8">{children}</div></article>;
}

function ServicosPage() {
  return <main>
    <section className="border-b border-border bg-card/50"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16"><p className="text-sm font-semibold uppercase tracking-[0.25em] text-gold">Gomes Motors</p><h1 className="mt-2 text-3xl font-bold text-foreground sm:text-4xl">Serviços</h1><p className="mt-3 max-w-3xl text-muted-foreground">Cada serviço tem um caminho próprio: escolha o veículo quando fizer sentido, preencha as informações necessárias e abra uma conversa organizada no WhatsApp.</p></div></section>
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8" aria-label="Serviços disponíveis"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{SERVICOS.map(({ id, icon: Icon, title, description }) => <a key={id} href={`#${id}`} className="group border border-border bg-card p-5 transition-colors hover:border-gold"><Icon className="h-7 w-7 text-gold" /><h2 className="mt-4 font-semibold text-foreground">{title}</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">{description}</p><span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-gold">Abrir fluxo <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" /></span></a>)}</div></section>
    <section className="border-y border-border bg-background"><div className="mx-auto max-w-7xl space-y-8 px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
      <ServicoBloco id="comprar" icon={Car} title="Comprar" detalhe="Se você já sabe qual veículo quer, selecione-o abaixo. O nome, ano e preço são levados para o WhatsApp junto com seus dados."><div className="mb-8 grid gap-4 border-b border-border pb-7 md:grid-cols-3"><Roteiro numero="1" texto="Escolha o carro ou a moto no estoque." /><Roteiro numero="2" texto="Informe nome, cidade e forma de pagamento." /><Roteiro numero="3" texto="Receba o resumo pronto no WhatsApp para continuar o atendimento." /></div><FormularioComprar /></ServicoBloco>
      <ServicoBloco id="vender" icon={BadgeDollarSign} title="Vender" detalhe="Preencha os dados do veículo, documentação, condição e contato. O resumo será organizado para facilitar a avaliação inicial pelo WhatsApp."><div className="mb-8 grid gap-4 border-b border-border pb-7 md:grid-cols-3"><Roteiro numero="1" texto="Informe as características e o histórico comercial do veículo." /><Roteiro numero="2" texto="Registre documentação, pendências e fotos que você pretende enviar." /><Roteiro numero="3" texto="Envie o veículo para avaliação e combine os próximos passos." /></div><FormularioVeiculo tipo="vender" /></ServicoBloco>
      <ServicoBloco id="trocar" icon={Repeat} title="Trocar" detalhe="Primeiro escolha o veículo desejado no estoque. Depois informe os dados do seu veículo atual para que os dois lados da negociação cheguem juntos ao WhatsApp."><div className="mb-8 grid gap-4 border-b border-border pb-7 md:grid-cols-3"><Roteiro numero="1" texto="Escolha o carro ou a moto que deseja." /><Roteiro numero="2" texto="Informe ano, km, condição, valor e financiamento do seu veículo atual." /><Roteiro numero="3" texto="Envie a intenção de troca com os dois veículos identificados." /></div><FormularioTroca /></ServicoBloco>
      <ServicoBloco id="consignar" icon={KeyRound} title="Consignar" detalhe="A consignação tem um fluxo próprio. Envie informações completas do veículo para conversar sobre viabilidade, documentação e condições comerciais."><div className="mb-8 grid gap-4 border-b border-border pb-7 md:grid-cols-3"><Roteiro numero="1" texto="Cadastre o veículo e o valor que você busca." /><Roteiro numero="2" texto="Informe condição, financiamento, documentação e fotos." /><Roteiro numero="3" texto="Envie a solicitação de consignação para alinhar as condições." /></div><FormularioVeiculo tipo="consignar" /></ServicoBloco>
      <ServicoBloco id="financiar" icon={CreditCard} title="Financiar" detalhe="Selecione um veículo, informe entrada e prazo e veja uma estimativa antes de pedir uma proposta real. A simulação não representa aprovação de crédito."><div className="mb-8 grid gap-4 border-b border-border pb-7 md:grid-cols-3"><Roteiro numero="1" texto="Escolha o veículo que deseja financiar." /><Roteiro numero="2" texto="Defina uma entrada e um prazo dentro das opções disponíveis." /><Roteiro numero="3" texto="Veja a parcela estimada e envie seus dados para uma proposta real." /></div><FormularioFinanciar /></ServicoBloco>
    </div></section>
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><div className="flex gap-4 border border-border bg-card p-6 sm:p-8"><FileText className="mt-1 h-6 w-6 shrink-0 text-gold" /><div><h2 className="font-semibold text-foreground">Sobre os formulários</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">As informações servem para organizar o primeiro contato comercial. Os dados não representam aprovação de financiamento, avaliação definitiva ou aceite de consignação. Condições reais devem ser confirmadas durante o atendimento.</p></div></div></section>
  </main>;
}
